...
def bulk_assign_proxy(body: BulkAssignProxyBody):
    ...
    for i, pid in enumerate(body.profile_ids):
        ...
        prof["proxy_username"] = px.get("username", "") or ""
        prof["proxy_password"] = px.get("password", "") or ""
...
def api_import_proxies(body: ProxyImportBody):
    ...
    for line in body.lines.splitlines():
        ...
        if "://" in line:
            line = line.split("://", 1)[1]
        # hỗ trợ 2 dạng: host:port  hoặc  host:port:username:password
        parts = line.split(":")
        if len(parts) < 2:
            continue
        host = parts[0].strip()
        try:
            ...
            continue
        px_user = parts[2].strip() if len(parts) >= 4 else ""
        px_pass = parts[3].strip() if len(parts) >= 4 else ""
        key = f"{host}:{port}"
