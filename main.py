                        ...
                        if card_number:
                            ...
                            for _pay_retry in range(5):  # tối đa 5 thẻ/phiên
                              ...
                              if not _was_skip:
                                ...
                                # Lưu ý: toàn bộ block này nằm trong try/except riêng → lỗi
                                # bất kỳ KHÔNG dừng flow, chỉ log cảnh báo và tiếp tục điền thẻ.
                                try:
                                    _link_dismissed = False
                                    # Selector chính xác cho nút "Continue with Link"
                                    _CWL_SEL = (
                                        'button:has-text("Continue with Link"), '
                                        'a:has-text("Continue with Link")'
                                    )
                                    # Các nút dismiss phổ biến mà Stripe Link hiển thị
                                    _CWL_CANCEL_SELS = [
                                        'button:has-text("Not now")',
                                        'button:has-text("No thanks")',
                                        'button:has-text("Use a different card")',
                                        'button:has-text("Use different card")',
                                    ...
                                    for _ld_attempt in range(3):
                                        # Tìm nút "Continue with Link" — main page trước, rồi tất cả frames
                                        _link_btn = None
                                        _link_ctx = page  # context chứa _link_btn
                                        try:
                                            _link_btn = page.query_selector(_CWL_SEL)
                                        except Exception:
                                            pass
                                        if not _link_btn:
                                            for _fr in page.frames:
                                                try:
                                                    _b = _fr.query_selector(_CWL_SEL)
                                                    if _b:
                                                        _link_btn = _b
                                                        _link_ctx = _fr
                                                        break
                                                    ...
                                                    pass
                                        if not _link_btn:
                                            break  # không có "Continue with Link" → tiếp tục bình thường
                                        log(f"[{idx+1}] ⚠ Phát hiện 'Continue with Link' — đang dismiss (lần {_ld_attempt+1})...")
                                        _dismissed = False
                                        # Thử các nút dismiss trong cùng context với _link_btn trước
                                        for _ctx_try in [_link_ctx, page]:
                                            if _dismissed:
                                                break
                                            for _cancel_sel in _CWL_CANCEL_SELS:
                                                try:
                                                    _cancel = _ctx_try.query_selector(_cancel_sel)
                                                    if _cancel and _cancel.is_visible():
                                                        _cancel.click()
                                                        page.wait_for_timeout(800)
                                                        _dismissed = True
                                                        log(f"[{idx+1}] ✓ Đã click dismiss Stripe Link: '{_cancel_sel}'")
                                                        break
                                                except Exception:
                                                    pass
                                        if not _dismissed:
                                            ...
                                            try:
                                                ...
                                                pass
                                        if not _dismissed:
                                            # Fallback 2: click vùng ngoài overlay (góc trái trên)
                                            try:
                                                page.mouse.click(10, 10)
                                                page.wait_for_timeout(700)
                                                _dismissed = True
                                                log(f"[{idx+1}] ✓ Đã click-outside Stripe Link overlay")
                                            except Exception:
                                                pass
                                        if _dismissed:
                                            page.wait_for_timeout(800)
                                    ...
                                    log(f"[{idx+1}] ⚠ Lỗi dismiss Stripe Link (bỏ qua, tiếp tục): {_ld_ex}")
